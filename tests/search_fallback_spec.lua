...
local success_body = [[{"issues":[{"key":"SPL-106","fields":{"summary":"Multi Directory with multi type configuration","status":{"name":"Done"}}}],"total":1,"startAt":0,"maxResults":50,"nextPageToken":"next-fallback"}]]
local decoded_bodies = {
  [success_body] = {
    issues = {
      { key = "SPL-106", fields = { summary = "Multi Directory with multi type configuration", status = { name = "Done" } } },
    },
    total = 1,
    startAt = 0,
    maxResults = 50,
    nextPageToken = "next-fallback",
  },
}
...
assert(result.issues and result.issues[1] and result.issues[1].key == "SPL-106", "expected SPL-106 in search results")
assert(result.issues[1].summary == "Multi Directory with multi type configuration", "expected SPL-106 summary to match payload")
assert(result.next_page_token == "next-fallback", "expected next page token from search response")
assert(#captured_calls == 1, "expected a single request to /search")
assert(captured_calls[1].url:find("/rest/api/3/search", 1, true), "expected request to /search")
assert(captured_calls[1].body == [[{"jql":"key = \"SPL-106\"","maxResults":50,"fields":["key","summary","status"]}]], "unexpected search payload: " .. tostring(captured_calls[1].body))

_G.vim = original_vim
io.open = original_io_open
print("ok\tjira.api.search_issues hits /search and returns SPL-106 summary")
